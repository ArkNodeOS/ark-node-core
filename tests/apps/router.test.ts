...
const mockExecSync = mock((cmd: string): string => {
	if (
		cmd.includes("ip route show default") &&
		cmd.includes("awk '/default/ {print $3}'")
	)
		return "192.168.1.1";
	if (
		cmd.includes("ip route show default") &&
		cmd.includes("awk '/default/ {print $5}'")
	)
		return "eth0";
...
(globalThis as any).fetch = async (url: string) => {
	if (url.toString().includes("ipify.org"))
		return new Response("1.2.3.4", { status: 200 });
